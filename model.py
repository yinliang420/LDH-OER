...
class EquiformerV2_OC20(BaseModel):
    ...
    def f_forward(self, data):
        if data.natoms is str:
            self.batch_size = len(data.natoms)
        else:
    ...
    def l_forward(self, x, data, atomic_numbers, edge_distance, edge_index):
        ###############################################################
        # Energy estimation
        ###############################################################
        node_energy = self.energy_block(x)
        # print(self.energy_block.so3_linear_1.expand_index)
        # print(self.energy_block.so3_linear_2.expand_index)
        node_energy = node_energy.embedding.narrow(1, 0, 1)
    ...
    def finalfforward(self, data):
        ...
        ###############################################################
        node_energy = self.energy_block(x)
        return node_energy, atomic_numbers

    def finallforward(self, node_energy, atomic_numbers, data):
        node_energy = node_energy.embedding.narrow(1, 0, 1)
        energy = torch.zeros(
            len(data.natoms),
            device=node_energy.device,
    ...
    def first_lforward(self, data, x, atomic_numbers, edge_distance, edge_index):
        for i in range(self.num_layers):
            x = self.blocks[i](
                x,  # SO3_Embedding
                atomic_numbers,
...
@registry.register_model("equiformerV2_f")
class EquiformerV2_f(nn.Module):
    def __init__(self,
                 model,
                 layer_output,
                 dropout1: float = 0.5,
        ...
        self.layer1 = nn.Linear(128, layer_output)
        self.layer2 = nn.Linear(layer_output, 128)
        self.dropout_1 = nn.Dropout(p=dropout1)
        self.dropout_2 = nn.Dropout(p=dropout2)

    def forward(self, data):
        x, atomic_numbers, edge_distance, edge_index = self.model.f_forward(data)
        # import torch.nn.functional as F

        x.embedding = self.layer1(x.embedding)
        x.embedding = torch.sigmoid(x.embedding)
        x.embedding = self.dropout_1(x.embedding)
        x.embedding = self.layer2(x.embedding)
        x.embedding = self.dropout_2(x.embedding)

        return self.model.l_forward(x, data, atomic_numbers, edge_distance, edge_index)

    def get_tensor(self, data):
        return self.model.get_tensor(data)


@registry.register_model("equiformerV2_final")
class EquiformerV2_final(nn.Module):
    def __init__(self,
                 model,
                 layer_output,
                 dropout1: float = 0.5,
        ...
        self.layer1 = nn.Linear(49, layer_output)
        self.layer2 = nn.Linear(layer_output, 49)
        self.dropout_1 = nn.Dropout(p=dropout1)
        self.dropout_2 = nn.Dropout(p=dropout2)

    def forward(self, data):
        node_energy, atomic_numbers = self.model.finalfforward(data)  # 原子数*49*1
        node_energy.embedding = node_energy.embedding.squeeze(-1)
        node_energy.embedding = self.layer1(node_energy.embedding)
        node_energy.embedding = torch.sigmoid(node_energy.embedding)
        node_energy.embedding = self.dropout_1(node_energy.embedding)
        node_energy.embedding = self.layer2(node_energy.embedding)
        node_energy.embedding = self.dropout_2(node_energy.embedding)
        node_energy.embedding = node_energy.embedding.unsqueeze(-1)

        return self.model.finallforward(node_energy, atomic_numbers, data)

    def get_tensor(self, data):
        return self.model.get_tensor(data)


@registry.register_model("equiformerV2_first")
class EquiformerV2_first(nn.Module):
    def __init__(self,
                 model,
                 layer_output,
                 dropout1: float = 0.5,
    ...
    def forward(self, data):
        x, atomic_numbers, edge_distance, edge_index = self.model.first_fforward(data)
        # import torch.nn.functional as F
        ...
        return self.model.first_lforward(data, x, atomic_numbers, edge_distance, edge_index)
