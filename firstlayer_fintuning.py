...
from ocpmodels.models.equiformer_v2.equiformer_v2_oc20 import EquiformerV2_f, EquiformerV2_OC20, EquiformerV2_first
...
def train(model, data_loader, optimizer, num_epochs, criterion, normalizers, device, save_best: bool,
        ...
        # for phase in ['train']:
            ...
            if phase == 'valid' and epoch_loss < best_loss:
                ...
                if save_best is True:
                    ...
                    filename = f'path_to_save\\doping.pt'
    ...
    if save_best is True:
        print("firstlayerTrain_loss: ", train_losses)
        print("firstlayerValid_losses: ", valid_losses)
        print("firstlayerBest_loss: ", best_loss, "\t", valid_losses.index(best_loss))
    else:
...
if __name__ == "__main__":
    ...
    for nums in numlist:
        ...
        for fold, (train_index, valid_index) in enumerate(kf.split(split_data)):
            ...
            lr_dropout1 = 0.5
            lr_dropout2 = 0.5
            my_model = EquiformerV2_first(equiformer, layer_output, lr_dropout1, lr_dropout2)
            ...
            valid_loss = train(my_model, dataloader, optimizer, num_epochs, criterion, normalizers, device, True,
                               judge_force, fold, nums)
            fold_results.append(valid_loss)

        avg_valid_loss = sum(fold_results) / len(fold_results)
        print(f"Average validation loss across {num_folds} folds: {avg_valid_loss}")
        with open(f"vaildloss.txt", 'a')as f:
